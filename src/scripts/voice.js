...
async function initLiveKit() {
    ...
    try {
        ...
        // Cek apakah server ngasih token atau malah error
        if (!data.token) {
        ...
        // Listener untuk deteksi siapa yang lagi ngomong (Efek Sonar)
        ...
        // Listener supaya bisa denger suara orang lain
        ...
        // Standby mic mati dulu
        await room.localParticipant.setMicrophoneEnabled(false);

    } catch (e) { 
        console.error("❌ LiveKit Connection Error:", e.message);
        // Kalau error di sini, biasanya karena Token kadaluarsa atau URL salah
    }
}
...
async function sendGift(giftName, harga, giftId, jumlah = 1) {
    ...
    combo.syncTimer = setTimeout(async () => {
        ...
        try {
            ...
            // 🔥 [FIX] CATAT KE TABEL COIN_HISTORY (SISI PENGIRIM) 🔥
            await sb.from('coin_history').insert([{
                user_id: MY_USER_ID,
                transaction_type: 'send_gift',
                amount: -coinsToDeduct, // Minus karena koin keluar
                description: `Kirim ${giftName} x${currentCount} ke ${finalTargetName}`,
                balance_after: saldoSkrg // Saldo yang sudah kita hitung di awal UI instan
            }]);

            // 🔥 [FIX] CATAT KE TABEL COIN_HISTORY (SISI PENERIMA - OPSIONAL) 🔥
            await sb.from('coin_history').insert([{
                user_id: finalTargetId,
                transaction_type: 'receive_gift',
                amount: coinsToDeduct, // Plus karena koin masuk
                description: `Terima ${giftName} x${currentCount} dari ${myUsername}`,
                balance_after: 0 // Kita isi 0 dulu atau biarkan null karena kita ga tau saldo asli penerima saat ini
            }]);

            // --- UPDATE LEVEL & CHAT (Sisanya tetep sama) ---
...
async function toggleMicSidebar(event) {
    ...
    // Cek posisi panggung langsung dari UI (tanpa nunggu Database)
...
async function naikKeStage(index) {
    ...
    try {
        // 1. NYALAKAN MIC DULU SAAT KLIK (Biar lolos sistem keamanan browser)
        if (room && room.state === "connected") {
            await room.localParticipant.setMicrophoneEnabled(true);
        }

        // 2. Cek apakah kursi kosong
        ...
        if (checkSlot && checkSlot.profile_id !== null) {
            // Kalau penuh, matikan mic-nya lagi dan batal naik
            await room.localParticipant.setMicrophoneEnabled(false);
            return alert("Kursi sudah ada yang menempati!");
        }
        
        // 3. Update Database (Ambil Kursi & Hapus Ikon Mute)
...
async function prosesTurunMic() {
    ...
    try {
        // 1. Matikan Mic Duluan (Biar instan!)
        if (room && room.localParticipant) {
            await room.localParticipant.setMicrophoneEnabled(false);
        }

        // 2. Kosongkan Kursi di Database
        await sb.from('room_slots').update({ profile_id: null }).eq('profile_id', MY_USER_ID);
        
        // 3. Update Status Mute di DB
        await sb.from('profiles').update({ mic_off: true }).eq('id', MY_USER_ID);

        // 4. Reset Tampilan Sidebar
        const micIcon = document.getElementById('mic-icon');
        const micText = document.getElementById('mic-text');
        if(micIcon) { micIcon.innerText = "mic_off"; micIcon.style.color = "#e74c3c"; }
        if(micText) micText.innerText = "Hidupkan Mic";
